...
class AQSOLoader(BaseDataLoader):
    """AQSOL data loader with multi-task support and dataset splits."""
    
    # AQSOL dataset info
    AQSOL_PROPERTIES = ["solubility"]
    ...
    def __init__(self, config: ProjectConfig, 
        ...
        super().__init__("aqsol", config, target_property)
        # logger.error(f"AQSOL dataset not yet complete")
        # raise NotImplementedError(f"AQSOL dataset not yet complete")
    ...
    def _load_processed_data(self) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]]]:
        logger.info(f"Loading AQSOL data from: {self.data_dir}")
    ...
    def _extract_labels(self, data: List[Dict[str, Any]]) -> List[Any]:
        ...
        for sample in data:
            label = sample['properties']['solubility']
            labels.append(label)
    ...
    def _build_attribute_cache(self, processed_data: List[Dict[str, Any]]) -> None:
        ...
        for sample in tqdm(processed_data, desc="Building cache"):
            ...
            node_cache = {}
            # AQSOL format: one discrete value per node (atomic number)
            assert 'feat' in dgl_graph.ndata, f"Graph {graph_id} missing node features"
            ...
            # AQSOL format: one discrete value per edge (bond type)
            edge_cache = {}
            assert 'feat' in dgl_graph.edata, f"Graph {graph_id} missing edge features"
    ...
    def get_node_types_bulk(self, graph: dgl.DGLGraph, node_ids: List[int]) -> List[str]:
        ...
        type_ids = graph.ndata['feat'][ids].long().tolist()
        return [self.ATOM_TYPES[int(t)] for t in type_ids]
    ...
    def get_edge_types_bulk(self, graph: dgl.DGLGraph, edge_ids: List[int]) -> List[str]:
        ...
        type_ids = graph.edata['feat'][ids].long().tolist()
        return [self.BOND_TYPES[int(t)] for t in type_ids]
    ...
    def get_split_indices(self) -> Dict[str, List[int]]:
        ...
        try:
            ...
            logger.error(f"Failed to load index files: {e}")
            raise 