...
class AQSOLoader(BaseDataLoader):
    ...
    def __init__(self, config: ProjectConfig, 
        ...
        # logger.error(f"AQSOL数据集尚未完成")
        # raise NotImplementedError(f"AQSOL数据集尚未完成")
    ...
    def _load_processed_data(self) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], List[Dict[str, Any]]]:
        ...
        """
        logger.info(f"📂 从预处理数据目录加载AQSOL数据: {self.data_dir}")
    ...
    def _build_attribute_cache(self, processed_data: List[Dict[str, Any]]) -> None:
        ...
        for sample in tqdm(processed_data, desc="构建缓存"):
            ...
            # 构建节点属性缓存
            node_cache = {}
            # aqsol格式：每个node一个离散数值，即原子序数。graph.ndata：tensor(num_nodes),eg:[1,6,6,6,1,8,1]。
            # 因此，node_id即为原子序数。
            assert 'feat' in dgl_graph.ndata, f"❌ 图{graph_id}没有节点特征"
            ...
            # 构建边属性缓存
            # aqsol格式：每个edge一个离散数值，即键类型。graph.edata：tensor(num_edges),eg:[1,2,2,2,1]。
            # 因此，edge_id即为键类型。
            edge_cache = {}
            assert 'feat' in dgl_graph.edata, f"❌ 图{graph_id}没有边特征"
