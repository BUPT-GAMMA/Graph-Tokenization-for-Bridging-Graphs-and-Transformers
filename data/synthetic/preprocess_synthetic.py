...
def main() -> None:
    ...
    for i in range(len(ds)):
        ...
        if 'node_labels' not in g.ndata:
            raise RuntimeError('SYNTHETIC 缺少 node_labels')
        ...
        # 边 token: 不存在 → 0
        e = g.num_edges()
        edge_tok = torch.zeros(e, dtype=torch.long)
        g.edata['edge_token_ids'] = edge_tok.view(-1, 1)
        g.edata['edge_type_id'] = edge_tok
