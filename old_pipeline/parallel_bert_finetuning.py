...
1. 基本使用（自动检测GPU）：
   python parallel_bert_finetuning.py --dataset qm9

2. 指定GPU数量：
   python parallel_bert_finetuning.py --dataset qm9 --gpu_count 4

3. 指定特定方法：
   python parallel_bert_finetuning.py --dataset qm9 --methods eulerian feuler

4. 只微调原始数据（不微调BPE）：
   python parallel_bert_finetuning.py --dataset qm9 --skip_bpe

5. 只微调BPE数据（不微调原始数据）：
   python parallel_bert_finetuning.py --dataset qm9 --skip_raw

6. 自定义微调参数：
   python parallel_bert_finetuning.py --dataset qm9 --finetune_epochs 20 --finetune_batch_size 16
...
# 导入BERT微调模块
from bert_regression import finetune_bert_model_normalized
...
def get_pretrained_model_path(
    ...
    # 统一模型目录：直接在标准 model 目录层查找
    models_root = Path("model") / base_config.dataset.name
...
def run_single_bert_finetuning(args_tuple: Tuple[str, str, bool, ProjectConfig, str, int]) -> Dict[str, Any]:
    """
    运行单个BERT微调任务
    ...
    try:
        ...
        print(f"🔄 [{task_id}] 开始微调: {task_name} (设备: {actual_device})")
        ...
        model_path = finetune_bert_model_normalized(task_config, str(pretrained_path))
        training_time = time.time() - start_time
        
        print(f"✅ [{task_id}] 微调完成: {task_name} (耗时: {training_time:.2f}s)")
        ...
        print(f"❌ [{task_id}] 微调失败: {method}_{'bpe' if use_bpe else 'raw'} - {e}")
        import traceback
        traceback.print_exc()
        return {
            'task_id': task_id,
...
def run_single_bert_finetuning_with_queue(args_tuple: Tuple[str, str, bool, ProjectConfig, str, int], result_queue: mp.Queue):
    """
    运行单个BERT微调任务（使用Queue收集结果）
    
    Args:
        args_tuple: (dataset_name, method, use_bpe, base_config, device, task_id)
        result_queue: 结果队列
    """
    result = run_single_bert_finetuning(args_tuple)
    result_queue.put(result)

def run_parallel_bert_finetuning(
    dataset_name: str,
    ...
    if gpu_count > 0:
        ...
        for task in tasks:
            p = mp.Process(
                target=run_single_bert_finetuning_with_queue,
...
示例用法:
  ...
  # 指定目标属性
  python parallel_bert_finetuning.py --dataset qm9 --target_property homo
  ...
  python parallel_bert_finetuning.py --dataset qm9 --finetune_epochs 20 --finetune_batch_size 16
    ...
    parser.add_argument("--normalization_method", type=str, choices=['standard', 'minmax', 'robust'], help="标签标准化方法")
    ...
    if args.finetune_learning_rate:
        config.finetune_learning_rate = args.finetune_learning_rate
    ...
    # 运行并行微调
    results = run_parallel_bert_finetuning(
