...
本指南提供了将项目中的DGL格式数据集导出为**极简格式**的方案，每个数据集一个导出脚本，输出统一的简单格式，支持在其他项目中使用PyG或DGL格式。
...
| 数据集 | 导出脚本 | 节点特征 | 边特征 | 标签 |
|--------|----------|----------|---------|-------|
| QM9 | `export_qm9.py` | 原子序数 | 键类型 | 分子属性字典 |
| ZINC | `export_zinc.py` | 原子序数 | 键类型 | 回归数值 |
| MOLHIV | `export_molhiv.py` | 原子序数 | 键类型 | 分类标签 |
