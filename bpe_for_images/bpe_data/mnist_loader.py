...
def _build_flatten_index(order: str, h: int = 28, w: int = 28) -> List[int]:
    """生成不同展平策略的索引顺序。
    order: 'row'|'col'|'snake'|'diag'
    返回长度 h*w 的一维索引数组（按行优先的扁平索引）。
    """
    ...
    if order == 'row':
        ...
        # 对角线扫描（左上->右下的对角线，之字形方向）
        # 生成所有对角线：r+c = const
        diagonals: List[List[int]] = [[] for _ in range(h + w - 1)]
        for r in range(h):
            for c in range(w):
                diagonals[r + c].append(r * w + c)
        # 之字形：偶数对角线正序，奇数对角线反序
...
class MNISTDataset(Dataset):
    ...
    def __init__(
        ...
        flatten_strategy: str = "row"  # row|col|snake|diag
    ):
        """
        Args:
            root: 数据根目录
        ...
        """
        self.format = format
        self.flatten_strategy = (flatten_strategy or 'row').lower()
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int]:
        if self.format == "sequence":
            ...
            # 根据策略重排
            idxs = _build_flatten_index(self.flatten_strategy, img_array.shape[0], img_array.shape[1])
            sequence = img_array.reshape(-1)[idxs]  # (784,)
            ...
            if self.format == "flatten":
                # (1,28,28)->(784,) 并按策略重排
                flat = img.view(-1)
                idxs = _build_flatten_index(self.flatten_strategy)
                img = flat[idxs]
            return img, label
...
def get_mnist_dataloaders(
    ...
    """
    # 加载数据集
    train_dataset = MNISTDataset(data_dir, train=True, download=True, format=format, flatten_strategy=flatten_strategy)
    test_dataset = MNISTDataset(data_dir, train=False, download=True, format=format, flatten_strategy=flatten_strategy)
...
def prepare_flattened_sequences(
    ...
    idxs = _build_flatten_index(strategy, images.shape[1], images.shape[2])
    for i in range(N):
        # 展平为(784,)并转换为Python int列表
        seq = images[i].reshape(-1)[idxs].astype(np.int64).tolist()
        sequences.append(seq)
    
    logger.info(f"准备展平序列: {N}个样本, 每个长度={len(sequences[0])}, 策略={strategy}")
